...
class _InventoryTypeSelectionScreenState extends State<InventoryTypeSelectionScreen> {
  ...
  Future<void> _loadCategoryCounts() async {
    try {
      ...
      // Contar equipos de cómputo desde v_equipos_computo_completo (misma vista que se usa en la pantalla)
      try {
        ...
        // Si falla con la vista, intentar contar desde t_computo_detalles_generales como fallback
        try {
          final equiposFallback = await supabaseClient
              .from('t_computo_detalles_generales')
              .select('id_equipo_computo');
          countMap['Equipo de Cómputo'] = equiposFallback.length;
        } catch (e2) {
          debugPrint('Error al contar equipos de cómputo (fallback): $e2');
